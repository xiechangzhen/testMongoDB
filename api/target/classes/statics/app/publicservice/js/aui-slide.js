...
(function(window) {
	...
	var	CLASS_SLIDER_NODE = "aui-slide-node",
		...
		CLASS_SLIDE_PAGE_TEXT = "aui-slide-page-text";
	...
	var __SLIDER_NODE = "."+CLASS_SLIDER_NODE,
		...
		__SLIDE_PAGE_TEXT = "."+CLASS_SLIDE_PAGE_TEXT;
	...
	auiSlide.prototype._init = function(options) {
		...
		this.textArray=this.options.textArray;//文字数组
	  this.textBgColor=this.options.textBgColor;//文字背景颜色
	...
	auiSlide.prototype.setPaginationActive = function(index){
		...
		var pageText = this.container.querySelector(__SLIDE_PAGE_TEXT);
		if(!pageText){
			return;
		}
		if(this.textArray.length>0)
		{
			pageText.innerHTML = this.textArray[this.index];
		}
	...
	// 创建文字分页器
	auiSlide.prototype.createText= function(index){
		var self = this;
		var pageText = this.container.querySelector(__SLIDE_PAGE_TEXT);
		if(!pageText){
			return;
		}
		pageText.innerHTML =this.textArray[0];
	};
	// 设置文字背景颜色
	auiSlide.prototype.setBgColor= function(){
		var self = this;
		var pageText = this.container.querySelector(__SLIDE_PAGE_TEXT);
		if(!pageText){
			return;
		}
		pageText.style.background =this.textBgColor;
	};
	// 总页数
